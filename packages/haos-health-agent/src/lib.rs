...
pub fn to_bounded_json(snapshot: &Snapshot) -> Result<String, String> {
    ...
    if text.len() > MAX_JSON_BYTES {
        return Err("health snapshot exceeds maximum size".into());
    }
    Ok(text)
}
fn uptime_ms() -> u128 {
...
fn load_average() -> Option<[f64; 3]> {
    #[cfg(unix)]
    {
        ...
        let mut it = text.split_whitespace().take(3).map(|x| x.parse().ok());
...
fn cgroup_snapshot() -> Option<CgroupSnapshot> {
    #[cfg(target_os = "linux")]
    {
        let current = read_value("/sys/fs/cgroup/memory.current").and_then(|x| x.parse().ok());
        let max = read_value("/sys/fs/cgroup/memory.max");
        let pc = read_value("/sys/fs/cgroup/pids.current").and_then(|x| x.parse().ok());
        let pm = read_value("/sys/fs/cgroup/pids.max");
        if current.is_none() && max.is_none() && pc.is_none() && pm.is_none() {
            None
        } else {
            Some(CgroupSnapshot {
...
mod tests {
    ...
    #[test]
    fn bounded() {
        let s = to_bounded_json(&snapshot(true)).unwrap();
        assert!(s.len() <= MAX_JSON_BYTES);
        assert!(s.contains("haos.health.v1"));
    }
    #[test]
    fn states() {
        assert!(snapshot(false).liveness);
        assert!(!snapshot(false).readiness);
        assert!(snapshot(true).readiness);
    }
}

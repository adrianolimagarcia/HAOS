...
pub fn read_snapshots(
    ...
    let start_col = first_column(&columns, &["worker_start_time", "worker_started_at", "pid_start_time"]);
    let sql = format!(
        "SELECT {id},{},{},{},{},{},{},{} FROM tasks{} ORDER BY {id}",
        pid.unwrap_or("NULL"), assignee.unwrap_or("NULL"), status.unwrap_or("NULL"),
        heartbeat.unwrap_or("NULL"), ownership.unwrap_or("NULL"), profile_col.unwrap_or("NULL"),
        start_col.unwrap_or("NULL"),
        profile_col.map(|c| format!(" WHERE {c} = ?1")).unwrap_or_default()
    );
    ...
    while let Some(row) = rows.next()? {
        ...
        out.push(WorkerSnapshot {
            task_id: row.get(0)?, pid: row.get(1)?, assignee: row.get(2)?,
            status: row.get::<_, Option<String>>(3)?.unwrap_or_else(|| "unknown".into()),
            heartbeat_at,
            ownership: row.get(5)?, profile: row.get(6)?,
            process: observe_process(row.get(1)?, row.get(7)?),
            heartbeat_lost: heartbeat_at.map(|t| now.saturating_sub(t) > max_idle_seconds).unwrap_or(true),
        });
    }
    Ok(out)
}
...
fn table_columns(conn: &Connection) -> rusqlite::Result<std::collections::HashSet<String>> {
    ...
    let columns = stmt.query_map([], |r| r.get::<_, String>(1))?.filter_map(Result::ok).collect();
    Ok(columns)
}
fn first_column<'a>(cols: &std::collections::HashSet<String>, names: &'a [&'a str]) -> Option<&'a str> {
...
fn observe_process(pid: Option<i64>, recorded_start: Option<String>) -> ProcessObservation {
    let Some(pid) = pid.filter(|p| *p > 1) else { return ProcessObservation { state: "absent", pid_reused: false }; };
    let Ok(stat) = std::fs::read_to_string(format!("/proc/{pid}/stat")) else {
        return ProcessObservation { state: "absent", pid_reused: false };
    };
    let fields: Vec<&str> = stat.rsplit_once(") ").map(|(_, rest)| rest.split_whitespace().collect()).unwrap_or_default();
    let reused = recorded_start.as_deref().zip(fields.get(19).copied()).map(|(a, b)| a != b).unwrap_or(false);
    ProcessObservation { state: if reused { "pid_reused" } else { "alive" }, pid_reused: reused }
}

pub fn unix_now() -> i64 { SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() as i64 }
...
mod tests {
    ...
    fn db(sql: &str) -> std::path::PathBuf {
        ...
        let c = Connection::open(&p).unwrap(); c.execute_batch(sql).unwrap(); p
    }
    #[test] fn absent_pid_and_lost_heartbeat() {
        ...
        assert_eq!(s[0].process.state, "absent"); assert!(s[0].heartbeat_lost);
    }
    #[test] fn profile_filter_is_read_only() {
        ...
        let s = read_snapshots(&p, Some("B"), 100, 30).unwrap(); assert_eq!(s[0].task_id, "b");
    }
    #[test] fn pid_reuse_marker_is_conservative() {
        let current = std::fs::read_to_string(format!("/proc/{}/stat", std::process::id())).unwrap();
        let start = current.rsplit_once(") ").unwrap().1.split_whitespace().nth(19).unwrap();
        assert!(observe_process(Some(std::process::id() as i64), Some("different".into())).pid_reused);

...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = product_command("verify"),
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``haos verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command=product_command("verify"), kind="verify",

...
_RELOAD_HTTP_ERRORS = {
    ...
    401: "management API rejected our key (401).  The running daemon was started with a different management.token — run `" + product_command("egress") + " restart`.",
}
...
def reload_proxy() -> bool:
    ...
    if not (pid := _read_pid()) or not _pid_alive(pid):
        raise RuntimeError("iron-proxy is not running — nothing to reload.  Run `" + product_command("egress") + " start`.")
    if (mgmt := _read_management_listen_from_config()) is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written before reload support).  Re-run `" + product_command("egress") + " setup` and use `" + product_command("egress") + " restart` this one time."
        )
    if not (token := _read_text_or_none(_proxy_state_dir_ro() / "management.token")):
        raise RuntimeError("management.token is missing — re-run `" + product_command("egress") + " setup`, then `" + product_command("egress") + " restart`.")
    host, port = mgmt
    ...
    try:
        ...
        raise RuntimeError(
            f"could not reach the management API at {host}:{port} ({exc}).  If the daemon was started before reload support, run `{product_command('egress')} restart` once."
        ) from exc
...
def start_proxy(
    *, binary: Optional[Path] = None, config_path: Optional[Path] = None, extra_env: Optional[Dict[str, str]] = None,
    ...
    if (bin_path := binary or find_iron_proxy(install_if_missing=install_if_missing)) is None:
        raise RuntimeError("iron-proxy binary not available — run `" + product_command("egress") + " install`.")
    if not (cfg := config_path or (_proxy_state_dir() / "proxy.yaml")).exists():
        raise RuntimeError(f"iron-proxy config not found at {cfg}. Run `{product_command('egress')} setup` first.")
...
def _write_pidfile_safely(pidfile: Path, pid: int) -> None:
    ...
    try:
        ...
        if (existing_pid := _read_pid()) and _pid_alive(existing_pid):
            raise RuntimeError(
                f"Another iron-proxy start appears to be in progress (pidfile {pidfile} -> pid {existing_pid}).  Run `{product_command('egress')} stop` if that proxy is stuck."
            )
        pidfile.unlink(missing_ok=True)
        fd = os.open(str(pidfile), open_flags, 0o600)
    except OSError as exc:
...
def _refresh_secrets_from_bitwarden(env: Dict[str, str], needed: set, bitwarden_config: Dict, allow_env_fallback: bool) -> None:
    ...
    if missing:
        _bitwarden_shortfall(
            ...
            f"credential_source: env via `{product_command('egress')} setup --no-bitwarden`, or set `proxy.allow_env_fallback: true` in "
    ...
    if warnings:  # log only the count: the taint analyzer can't tell bws status text is non-secret
        logger.warning("Bitwarden refresh produced %d warning(s); run `" + product_command("secrets") + " bitwarden status` for detail.", len(warnings))

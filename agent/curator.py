...
CURATOR_REVIEW_PROMPT = (
    ...
    "TRIGGER-FIRST FORMAT for every description you write or rewrite: open with "
    "\"Use when...\" naming the SITUATION that should retrieve the skill, not the "
    "skill's subject — \"Use when a SQLite write fails with 'database is locked'\", "
    "not \"About SQLite locking\". The trigger class has to land inside the "
    "truncated 57-char window, so it comes first and the topic comes after; a "
    "description that leads with the topic is invisible to the search that was "
    "supposed to find it.\n\n"

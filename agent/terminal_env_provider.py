...
class TerminalEnvironmentProvider(ProviderBase):
    ...
    * ``session_isolated_when_nonpersistent`` — non-persistent mode gives each session its own
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``haos setup`` after selection (the wizard persists ``terminal.backend`` itself)."""
        return []

    def post_setup(self) -> None:
        """Optional interactive hook run by ``haos setup`` after selection (prompt for tokens, install SDKs)."""

    def doctor_checks(self) -> List[Tuple[bool, str, str]]:
        """``haos doctor`` rows ``(ok, label, detail)``; default reflects :meth:`is_available`."""
        try:

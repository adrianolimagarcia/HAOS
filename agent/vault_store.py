...
# browser fill targets (agent/vault_login_classifier.py); the Desktop Add dialog and `haos vault add`
...
class VaultStore:
    ...
    def _locked(self):
        ...
        `haos vault add` and a TUI slash worker all write the same ``vault.json.enc``; two unlocked

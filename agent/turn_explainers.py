...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "replaced": (
        ...
        "(`" + _CLI + "{profile_arg}gateway stop`), run `" + _CLI + "{profile_arg}doctor` — not "
        "`" + _CLI + "{profile_arg}doctor --fix`, which would repair the wrong file in place — "
    ...
    "deleted_wal": (
        ...
        "message was not saved (a copy is kept in {home}/sessions/). Stop Hermes "
        "(`" + _CLI + "{profile_arg}gateway stop`), run `" + _CLI + "{profile_arg}doctor`, then start "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    ...
    "database. Close other Hermes windows, run `" + _CLI + "{profile_arg}doctor` to check "

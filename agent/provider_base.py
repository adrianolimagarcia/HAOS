...
Every tool-provider ABC shares the same identity + ``haos tools`` picker surface.
...
class ProviderBase(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``haos tools``. Defaults to ``name``."""
        return self.name

    def get_setup_schema(self) -> Dict[str, Any]:
        """Provider row for the ``haos tools`` picker.
...
class CatalogProviderBase(ProviderBase):
    ...
    @property
    def display_name(self) -> str:
        """Human-readable label shown in ``haos tools``. Defaults to ``name.title()``."""
    ...
    def is_available(self) -> bool:
        ...
        ``haos setup`` call it on every paint.
        """
        return True

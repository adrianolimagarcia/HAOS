...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run '" + product_command("setup") + " tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run '" + product_command("setup") + " tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run '" + product_command("tools") + "' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    (product_command("setup"), "          Re-run the full wizard"), (product_command("setup") + " model", "    Change model/provider"),
    (product_command("setup") + " terminal", " Change terminal backend"), (product_command("setup") + " gateway", "  Configure messaging"),
    (product_command("setup") + " tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    (product_command("config"), "         View current settings"), (product_command("config") + " edit", "    Open config in your editor"),
    (product_command("config") + " set <key> <value>", ""))
_READY_ROWS = (
    ("hermes", "              Start chatting"), (product_command("gateway"), "      Start messaging gateway"),
    (product_command("doctor"), "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run '" + product_command("setup") + "' to configure")
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run '" + product_command("setup") + " terminal'")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        ...
        _setup._info("  Finish this one step with either of:",
              "    " + product_command("model") + "            (pick any provider/model)",
              "    " + product_command("setup") + " --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run '" + product_command("setup") + " tools' to configure them,")

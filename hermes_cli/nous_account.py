...
FREE_TIER_NEEDS_ACCOUNT = "This needs a Nous account. Run `" + product_command("auth") + " upgrade`."
...
def format_nous_portal_entitlement_message(
    ...
    if account_info is None:
        return (
            ...
            f"Run `{product_command('model')}` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                ...
                f"paid access for {capability}. Log in with `{product_command('model')}` to enable Portal-managed "
                f"features. Billing and credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `{product_command('model')}`. "
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `" + product_command("model") + "` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            ...
            f"is unavailable. Run `{product_command('model')}` to authenticate again; if the problem persists, contact Nous support."
        )
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `" + product_command("model") + "` to refresh Hermes."
        return message
    return (
...
def _no_paid_access_message(
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f"member spend cap at {billing_url}, then run `{product_command('model')}` to refresh."
        )
    if active and paid:
        return (

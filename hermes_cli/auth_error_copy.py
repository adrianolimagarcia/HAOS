...
DEVICE_FLOW_ERROR_COPY = {
    ...
    "access_denied": (
        "Sign-in was declined in the browser. Run `{retry}` to try again, or `" + product_command("model") + "` to pick a "
        "different provider."),
    "invalid_grant": (
        "The sign-in code was not accepted by the server. Run `{retry}` to get a new code."),
    "invalid_client": (
        "The server did not recognize this copy of Hermes. Run `" + product_command("update") + "`, then `{retry}` again."),
}
...
def device_flow_error(code: str, description: str, *, retry_command: str = product_command("portal")) -> SignInCopyError:
...
def sign_in_failure_lines(
    exc: BaseException, *, service_host: str = "portal.nousresearch.com", retry_command: str = product_command("portal"),
) -> list:
    ...
    lead = _classify(
        exc, rules,
        "Could not sign in. Run `{retry}` to try again, or `" + product_command("model") + "` to pick a different provider.")
...
def provider_setup_failure_lines(exc: BaseException, *, retry_command: str = product_command("model")) -> list:

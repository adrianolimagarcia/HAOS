...
comments the legacy lines out. It runs from ``haos update`` for every profile home and from
``haos relay migrate`` for the active one.
"""
...
def migrate_profile_relay_env(home: Path, *, validate: bool = True) -> RelayMigrationResult:
    ...
    header = (
        ...
        f"# Generated by `{product_command('update')}` from the legacy HERMES_NEMO_RELAY_ATIF_*/ATOF_* variables,\n"
...
def print_relay_migration_report(results: list[RelayMigrationResult]) -> None:
    """Loud, actionable notice for `haos update` / `haos relay migrate`."""
...
def run_relay_migration_after_update() -> None:
    """`haos update` hook: migrate every profile home, print the notice. Best-effort by contract."""
    print_relay_migration_report(migrate_all_profile_relay_envs())


RELAY_MIGRATE_COMMAND = product_command("migrate") + " relay"


def cmd_migrate_relay(args) -> None:
    """``haos migrate relay [--all-profiles] [--no-validate]``."""

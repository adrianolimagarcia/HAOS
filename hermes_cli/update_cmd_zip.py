"""ZIP-download fallback for ``haos update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.
...
_STASH_HINT = "  Stash or commit your changes, then rerun `" + product_command("update") + "`."
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    try:
        ...
        print("  Re-run `" + product_command("update") + "` to retry; if the agent won't start, reinstall from https://hermes-agent.nousresearch.com")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `" + product_command("update") + " "
            f"--branch {branch}`, or update against main with `{product_command('update')}`."
        )
        _m().sys.exit(1)
    ...
    if not import_ok:
        ...
        print("  Re-run `" + product_command("update") + "` to complete it.")
        _m().sys.exit(1)

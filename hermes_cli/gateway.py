...
Handles: haos gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_ancestor_pids() -> set[int]:
    ...
    process or any of its parents. This prevents ``haos gateway status`` from falsely counting the
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``haos update``)."""
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``haos update``: ``--external-supervisor`` gateways
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}{product_command('gateway')} status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}{product_command('gateway')} restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: " + product_command("gateway") + " stop")
    else:
        print("⚠ Gateway process is running for this profile, but the service is not active")
        print(pids_line)
        print("  This is usually a manual foreground/tmux/nohup run, so `" + product_command("gateway") + "`")
...
def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``haos gateway status``."""
    try:
...
from hermes_constants import is_container, is_termux, is_wsl, product_command
...
def remove_legacy_hermes_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: " + product_command("gateway") + " migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo " + product_command("gateway") + " migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    " + product_command("gateway") + " uninstall")
    print_info("    sudo " + product_command("gateway") + " uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: " + product_command("gateway") + " install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``haos setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   " + product_command("gateway") + " run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: " + product_command("gateway"))
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: " + product_command("gateway") + " install")
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo " + product_command("gateway") + " uninstall --system")
    print_info("         " + product_command("gateway") + " install")
    print_info("         " + product_command("gateway") + " start")
...
def systemd_install(
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}{product_command('gateway')} start{scope_flag}              # Start the service")
    print(f"  {sudo}{product_command('gateway')} status{scope_flag}             # Check status")
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}{product_command('gateway')} install{scope_flag}")
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `" + product_command("gateway") + " status` or logs for final shutdown state."
        )
        return
...
def _systemd_reset_and_run(action: str, *, system: bool, previous_pid) -> None:
    ...
    try:
        ...
        print(
            ...
            "check `" + product_command("gateway") + " status` or logs for final state."
        )
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}{product_command('gateway')} restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}{product_command('gateway')} start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}{product_command('gateway')} restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}{product_command('gateway')} start{scope_flag}")
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``haos update`` hands back to
    ...
    ``--external-supervisor`` so ``haos update`` sees the flag on the live grandchild argv and hands the
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup haos gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"  Try manually: nohup {product_command('gateway')} run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  " + product_command("gateway") + " status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: " + product_command("gateway") + " start")
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: " + product_command("gateway") + " stop")
        else:
...
_PLATFORMS = [
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: " + product_command("pairing") + " generate bluebubbles",
...
def _prompt_unauthorized_access(platform_key: str) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Skipped — configure later with '" + product_command("gateway") + " setup'")
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `" + product_command("gateway") + " setup`.")
        return
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            print_info(
                "  Unknown DM users can request access and you approve them with `" + product_command("pairing") + " approve`."
            )
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            print_info("Start manually: " + product_command("gateway"))
    except UserSystemdUnavailableError as e:
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: " + product_command("gateway") + " run",
        "  For persistence:   tmux new -s hermes '" + product_command("gateway") + " run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: " + product_command("gateway") + " run",
        "  Or start it manually in the background (best effort): nohup " + product_command("gateway") + " run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: " + product_command("gateway") + " run",
    ),
}
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        print_info("  You can install later: " + product_command("gateway") + " install")
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo " + product_command("gateway") + " install --system")
        print_info("  Or run in foreground:  " + product_command("gateway") + " run")
        return
    try:
        ...
        print_info("  You can try manually: " + product_command("gateway") + " install")
...
def gateway_setup():
    ...
    if any(_is_progress(_platform_status(p)) for p in _all_platforms()):
        ...
        print()
        print_info("No platforms configured. Run '" + product_command("gateway") + " setup' when ready.")
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `{product_command('gateway')} {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_WSL_FOREGROUND_HINT = (
    "", "  " + product_command("gateway") + " run                              # direct foreground",
    "  tmux new -s hermes '" + product_command("gateway") + " run'         # persistent via tmux",
    "  nohup " + product_command("gateway") + " run > ~/.hermes/logs/gateway.log 2>&1 &  # background",
)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: " + product_command("gateway")),
    ("install", "wsl"): (1,
    ...
    ("install", "container"): (0,
        ...
        "To run the gateway: " + product_command("gateway") + " run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: " + product_command("gateway") + " run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: " + product_command("gateway") + " stop"),
    ("uninstall", "s6"): (None,
...
def _install_systemd_from_cli(args, *, force: bool, system: bool, run_as_user) -> None:
    if is_wsl():
        ...
        _print_info_lines(
            "  Consider running in foreground instead: " + product_command("gateway") + " run",
            "  Or use tmux/screen for persistence: tmux new -s hermes '" + product_command("gateway") + " run'",
        )
        print()
...
def _cmd_restart(args):
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    " + product_command("gateway") + " restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: " + product_command("gateway") + " start",
        )
        sys.exit(1)
...
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  " + product_command("gateway") + " install"),
    "other": (
        "To install as a service:", "  " + product_command("gateway") + " install", "  sudo " + product_command("gateway") + " install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    "termux": (
        "  nohup " + product_command("gateway") + " run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start",
    ),
    "wsl": (
        "  tmux new -s hermes '" + product_command("gateway") + " run'         # persistent via tmux",
        "  nohup " + product_command("gateway") + " run > ~/.hermes/logs/gateway.log 2>&1 &  # background",
    ),
    "windows": ("  " + product_command("gateway") + " install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  " + product_command("gateway") + " install  # Install as user service",
        "  sudo " + product_command("gateway") + " install --system  # Install as boot-time system service",
    ),
}
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        if pids:
            ...
            print()
            print("To start:")
            print("  " + product_command("gateway") + " run      # Run in foreground")

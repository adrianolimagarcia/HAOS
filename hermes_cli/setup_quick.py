...
def _run_portal_one_shot(config: dict) -> None:
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print()
        from hermes_cli.auth_error_copy import provider_setup_failure_lines
        lead, *rest = provider_setup_failure_lines(exc, retry_command=product_command("portal"))
        print_error(f"  {lead}")
        for line in rest:
            print_info(f"  {line}")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    def _on_error(exc: Exception) -> None:
        from hermes_cli.auth_error_copy import provider_setup_failure_lines
        lead, *rest = provider_setup_failure_lines(exc, retry_command=product_command("model"))
        print_warning(lead)
        for line in rest:
            print_info(line)

...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    newer one written by a concurrent ``haos gateway start`` (#110020 review)."""
    try:
...
# meant to bridge the seconds between a ✓ and the next ``haos gateway status``/``update``; a
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    one-shot marker — ``haos update`` consults it to decide whether a Desktop-owned install

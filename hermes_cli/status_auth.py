"""Credential sections of `haos status`, run through ``status._SECTIONS`` with its shared context.
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", product_command("model"), _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", product_command("auth") + " add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", product_command("auth") + " add xai-oauth", _FILE_REFRESH_ROWS))
...
def _render_auth_providers(ctx):
    ...
    _status._row("Nous Portal", logged_in,
         ...
         else "not logged in (run: " + product_command("portal") + ")")
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: " + product_command("model") + ")", 16, " ")

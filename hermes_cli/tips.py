...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or " + product_command("-c") + ".",
    ...
    # --- CLI Flags ---
    product_command("-c") + " resumes your most recent CLI session. " + product_command("-c") + " \"project name\" resumes by title.",
    product_command("-w") + " creates an isolated git worktree — perfect for parallel agent workflows.",
    product_command("-w", "-q", "\"Fix issue #42\"") + " combines worktree isolation with a one-shot query.",
    product_command("chat") + " -t web,terminal enables only specific toolsets for a focused session.",
    product_command("chat") + " -s github-pr-workflow preloads a skill at launch.",
    product_command("chat") + " -q \"query\" runs a single non-interactive query and exits.",
    product_command("chat") + " --max-turns 1000 overrides the default 500-iteration limit per turn.",
    product_command("chat") + " --checkpoints enables filesystem snapshots before every destructive file change.",
    product_command("--yolo") + " bypasses all dangerous command approval prompts for the entire session.",
    product_command("chat") + " --source telegram tags the session for filtering in " + product_command("sessions") + " list.",
    product_command("-p", "work", "chat") + " runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    product_command("doctor") + " --fix diagnoses and auto-repairs config and dependency issues.",
    product_command("dump") + " outputs a compact setup summary — great for bug reports.",
    product_command("config") + " set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    product_command("config") + " edit opens config.yaml in your default editor.",
    product_command("config") + " check scans for missing or stale configuration options.",
    product_command("sessions") + " browse opens an interactive session picker with search.",
    product_command("sessions") + " stats shows session counts by platform and database size.",
    product_command("sessions") + " prune --older-than 30 cleans up old sessions.",
    product_command("skills") + " search react --source skills-sh searches the skills.sh public directory.",
    product_command("skills") + " check scans installed hub skills for upstream updates.",
    product_command("skills") + " tap add myorg/skills-repo adds a custom GitHub skill source.",
    product_command("skills") + " snapshot export setup.json exports your skill configuration for backup or sharing.",
    product_command("mcp") + " add github --command npx adds MCP servers from the command line.",
    product_command("mcp") + " serve runs Hermes itself as an MCP server for other agents.",
    product_command("auth") + " add lets you add multiple API keys for credential pool rotation.",
    product_command("completion") + " bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    product_command("logs") + " -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    product_command("backup") + " creates a zip backup of your entire Hermes home directory.",
    product_command("profile") + " create coder creates an isolated profile that becomes its own command.",
    product_command("profile") + " create work --clone copies your current config and keys to a new profile.",
    product_command("update") + " syncs new bundled skills to ALL profiles automatically.",
    product_command("gateway") + " install sets up Hermes as a system service (systemd/launchd).",
    product_command("memory") + " setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    product_command("webhook") + " subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: " + product_command("tools") + " disables unused tools, " + product_command("skills") + " config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    product_command("models") + " routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as " + product_command("-w") + ").",
    ...
    "Profile names become shell commands — '" + product_command("profile") + " create coder' creates the 'coder' command.",
    product_command("profile") + " export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    product_command("sessions") + " export backup.jsonl exports all sessions for backup or analysis.",
    product_command("-r", "SESSION_ID") + " resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    product_command("skills") + " install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: " + product_command("cron") + " add --skill blogwatcher \"Check for new posts\".",
    ...
    product_command("gateway") + " install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    product_command("mcp") + " opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    product_command("mcp") + " catalog lists Nous-approved MCP servers shipped with the repo.",
    product_command("mcp") + " install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — " + product_command("tools") + " can toggle them per platform.",
    ...
    product_command("chat") + " -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    product_command("plugins") + " install owner/repo installs plugins directly from GitHub.",
    ...
    product_command("acp") + " runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    product_command("auth") + " add nous or " + product_command("auth") + " add openai-codex sets up OAuth-based providers.",
    ...
    product_command("status") + " --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — '" + product_command("profile") + " create ls' would be rejected.",
    product_command("profile") + " create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    product_command("claw") + " migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via " + product_command("tools") + ".",
    ...
    # --- Curator ---
    product_command("curator") + ' run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    product_command("curator") + " pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    product_command("curator") + ' rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
product_command("auth") + ' reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    product_command("dashboard") + ' launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    product_command("dashboard") + ' embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    product_command('-z', '"<prompt>"') + ' is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    product_command("chat") + ' --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    product_command("chat") + ' --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    product_command("chat") + ' --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    product_command("chat") + " --source tool tags programmatic chats so they don't clutter " + product_command("sessions") + " list.",
    product_command("dump") + ' --show-keys includes redacted API key fingerprints for deeper support debugging.',
    product_command("sessions") + ' rename <ID> "new title" renames any past session; ' + product_command("sessions") + ' delete <ID> removes one.',
    product_command("import") + ' restores a full Hermes backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    product_command("fallback") + ' manages the fallback_model chain interactively — no hand-editing config.yaml.',
    product_command("pairing") + ' rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    product_command("setup") + ' walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    product_command("status") + ' --deep runs the full health sweep across every component; plain ' + product_command("status") + ' is the quick view.',
    ...
    '--source tool chats are excluded from ' + product_command("sessions") + ' list by default — set --source explicitly to see them.',

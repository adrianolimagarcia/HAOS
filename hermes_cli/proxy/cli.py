"""CLI handlers for the ``haos proxy`` subcommand."""
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err(product_command("proxy") + " requires aiohttp. Run `" + product_command("setup") + "` to install it.")
        return 1
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"{product_command('auth')} add {adapter.name}")
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("\nStart the proxy with: " + product_command("proxy") + " start [--provider <name>]")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``haos proxy <subcommand>``; no/unknown subcommand prints the short help."""
    handler = _SUBCOMMANDS.get(getattr(args, "proxy_command", None))
    if handler is not None:
        return handler(args)
    _err(
        product_command("proxy") + " — local OpenAI-compatible proxy that attaches your\n" +
        "OAuth-authenticated provider credentials to outbound requests.\n" +
        "\n" +
        "Subcommands:\n" +
        "  " + product_command("proxy") + " start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n" +
        "      Run the proxy in the foreground.\n" +
        "  " + product_command("proxy") + " status\n" +
        "      Show which upstream adapters are ready.\n" +
        "  " + product_command("proxy") + " providers\n" +

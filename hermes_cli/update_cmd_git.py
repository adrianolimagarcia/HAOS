"""Git plumbing for ``haos update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && {product_command('update')}\n"
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped HERMES_HOME (``<root>/profiles/<name>``), so a profile-scoped ``haos update`` must look
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``haos update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``haos update``, which ships with the checkout

"""haos claw — OpenClaw migration commands."""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    if connected and _warn_running(
        ...
        ("Migrating bot tokens while the gateway is active will cause "
         ...
         "session per token).",
         "Recommendation: stop the gateway first with '" + product_command("gateway") + " stop'."),
...
def _cmd_migrate(args):
    ...
    if not opts.source_dir.is_dir():
        return _error_block(
            ...
            "You can specify a custom path: " + product_command("claw") + " migrate --source /path/to/.openclaw")
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    restorable with `haos import`: one restore point before any mutation, pruned to the last 5."""
    ...
    if not opts.no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: {product_command('import')} {backup_archive.name}")
        except Exception as e:
            return _error_block(
    ...
    try:
        ...
        if backup_archive:
            _info(f"A pre-migration backup is available at: {backup_archive}",
                  f"Restore with: {product_command('import')} {backup_archive.name}")
        return
...
def _cmd_cleanup(args):
    ...
    if running and not _warn_running(
        ...
        "Proceed anyway?",
        declined="Aborted. Stop OpenClaw first, then re-run: " + product_command("claw") + " cleanup",
        ...
        if dry_run:
            ...
            print_info(f"Would archive: {source_dir} → {archive_path}")
                      ...
                      non_tty=(f"Non-interactive session — would archive: {source_dir}",
                               "To execute, re-run with: " + product_command("claw") + " cleanup --yes")):
            try:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        _info("", "To execute the migration, run without --dry-run:",
              f"  {product_command('claw')} migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if any(i.get("kind") == "provider-keys" and i.get("status") == "skipped" for i in items):
            ...
            _info("", "To migrate API keys, re-run with:",
                  "  " + product_command("claw") + " migrate --migrate-secrets", "", "Or add your key manually:",
                  "  " + product_command("config") + " set OPENROUTER_API_KEY sk-or-v1-...")

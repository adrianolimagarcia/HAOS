"""``haos gateway migrate --multiplex`` / ``--standalone``: move a per-profile-gateway install onto one
...
what the multiplexer would do at startup. ``haos update`` calls :func:`maybe_auto_migrate_after_update`.
"""
...
MIGRATE_COMMAND = product_command("gateway") + " migrate --multiplex"
...
class MigrationPlan:
    ...
    def eligible_for_migration(self) -> bool:
        ...
        This is the AUTO-migration (``haos update``) bar; the explicit command also proceeds with
...
def _write_multiplex_flag(default_home: Path, value: bool) -> None:
    ...
    (same read-guard + nested-set + atomic write ``haos config set`` uses; no raw YAML edits)."""
...
def build_migration_plan() -> MigrationPlan:
    ...
    plan.notices.extend(f"Not migrated automatically by `{product_command('update')}`: {b}" for b in auto_migration_blockers(plan))
    plan.notices.append(
...
def format_plan(plan: MigrationPlan, *, dry_run: bool) -> list[str]:
    ...
    lines.append(f"  - record the previous state in {plan.default_home / MANIFEST_NAME} (rollback: " + product_command("gateway") + " migrate --standalone)")
    return lines + _plan_tail(plan)
...
def _no_manifest_lines(default_home: Path) -> list[str]:
    return [f"✗ No migration manifest at {_manifest_path(default_home)}; nothing to roll back.",
            "  To leave multiplex mode by hand: " + product_command("config") + " set gateway.multiplex_profiles false && " + product_command("gateway") + " restart"]
...
def format_update_warning(plan: MigrationPlan, auto_blockers: list[str]) -> list[str]:
    return [
        ...
        *[f"    • {b}" for b in (*plan.blockers, *auto_blockers)],
        ...
        "  (`" + product_command("update") + "` will migrate automatically once nothing blocks it.)",
    ]
...
def apply_migration(plan: MigrationPlan, *, served_wait: float = _SERVED_WAIT_SECONDS) -> bool:
    ...
    if served is not None and expected <= set(served):
        _print(["", f"✓ Migrated: the default gateway now serves {len(served)} profiles: {', '.join(served)}",
                f"  Rollback any time with: " + product_command("gateway") + " migrate --standalone",
    ...
    _print(["", f"⚠ Migration applied, but the default gateway has not confirmed serving: {', '.join(missing)}",
            "  Check `" + product_command("gateway") + " status` and the gateway log; the flag and manifest are in place.",
            "  Rollback: " + product_command("gateway") + " migrate --standalone"])
    return False
...
def rollback_migration(default_home: Optional[Path] = None) -> bool:
    ...
    if default_gw.has_gateway:
        try:
            ...
            print("    The default gateway is still multiplexing; stop it by hand (" + product_command("gateway") + " stop) and re-run.")
    if ok:
...
def _host_supports_migration() -> Optional[str]:
    ...
    if gw.is_windows():
        return "Windows Scheduled Tasks are not migrated automatically; set gateway.multiplex_profiles true, stop the per-profile tasks, and `" + product_command("gateway") + " restart`."
    return None


def cmd_migrate(args) -> None:
    """``haos gateway migrate [--multiplex|--standalone] [--dry-run] [--yes]``."""
...
def maybe_auto_migrate_after_update() -> None:
    """``haos update`` hook: with >= 2 profiles, per-profile gateways present and multiplex off,

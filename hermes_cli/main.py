...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    try:
        ...
        # classic installs at ~/.hermes. Try the product-native layout first so
        # `sudo haos -p <name>` finds the invoking user's profiles on HAOS too.
        stores = (".haos", ".hermes") if is_haos_environment() else (".hermes", ".haos")
        for store in stores:
            candidate = home / store / "profiles" / name
            if candidate.is_dir():

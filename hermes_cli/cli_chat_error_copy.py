...
_REASON_COPY: dict[str, str] = {
    "auth": "Your {provider} key was rejected. Run `" + product_command("model") + "` to re-enter it.",
    "auth_permanent": "Your {provider} key was rejected. Run `" + product_command("model") + "` to re-enter it.",
...
_UNKNOWN_COPY = "The model request failed. Run /model to switch or `" + product_command("doctor") + "` to check the setup."
...
def agent_init_failure_message(error: BaseException) -> str:
    ...
    return (
        ...
        f"Your message was not sent. Run `{product_command('doctor')}` to check the setup, "

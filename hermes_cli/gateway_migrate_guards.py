"""Boundaries the AUTOMATIC multiplex migration (``haos update``) must not cross, and the opt-out.
...
``haos gateway migrate --multiplex``; an unattended update hook must not choose it for them.
...
def auto_migration_blockers(plan: MigrationPlan) -> list[str]:
    ...
    domain, one profiles/ tree — the only shape ``hermes/haos update`` may fold on its own."""
    findings = [
        finding

"""HERMES_HOME state checks for haos doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'" + product_command("sessions") + " optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run '" + product_command("sessions") + " optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run '" + product_command("doctor") + " --fix' (or '" + product_command("sessions") + " repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run '" + product_command("doctor") + " --fix' (or '" + product_command("sessions") + " repair') to recover hidden sessions"),
}
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run '" + product_command("sessions") + " optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            if not should_fix:
                return f.issues.append("Large WAL file — run '" + product_command("doctor") + " --fix' to checkpoint")
            ...
            _SKIP = ("Large WAL file — cannot prove state.db is quiet (stop the profile's gateway first, then "
                     "re-run '" + product_command("doctor") + " --fix' to checkpoint)")
            if live_writer_holds_db(state_db_path, connect_repair_durable=_connect_repair_durable):
                ...
                check_warn("WAL checkpoint skipped: cannot prove state.db is quiet",
                           ...
                           "and re-run '" + product_command("doctor") + " --fix')")
            ...
            with _exclusive_repair_db_guard(state_db_path) as (guard, guard_error):
                if guard is None:
                    check_warn("WAL checkpoint skipped: could not take exclusive ownership of state.db",
                               "(" + str(guard_error) + "; stop the profile's gateway and re-run '"
                               + product_command("doctor") + " --fix')")
                    return f.issues.append(_SKIP)
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    ...
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: " + product_command("skills") + " list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: " + product_command("memory") + " setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: " + product_command("memory") + " setup",
                        "No Honcho API key — run '" + product_command("memory") + " setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run " + product_command("memory") + " setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: " + product_command("memory") + " status")
    else:
        check_warn(f"{name} plugin not found", "run: " + product_command("memory") + " setup")

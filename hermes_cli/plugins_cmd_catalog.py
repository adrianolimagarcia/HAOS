"""``haos plugins`` catalog surface: resolution, provenance sidecar, search/browse/info/validate,
...
def resolve_catalog_name(identifier: str, console) -> PluginCatalogEntry:
    ...
    if entry is None:
        _fail(console, (
            ...
            "owner/repo shorthand. Browse entries with `" + product_command("plugins") + " search`."))
...
def repin_catalog_plugin(target: Path, sidecar: dict) -> tuple[str, bool]:
    ...
    if entry is None:
        raise PluginOperationError(
            ...
            "See `" + product_command("plugins") + " info` and the removed blocklist.")
...
def _render_entries(entries: List[PluginCatalogEntry], console) -> None:
    ...
    console.print(table)
    console.print()
    console.print("[dim]Details:[/dim] " + product_command("plugins") + " info <name>    [dim]Install:[/dim] " + product_command("plugins") + " install <name>")
...
def cmd_info(name: str) -> None:
    ...
    if removed is not None:
        ...
        console.print()
    console.print(f"[dim]Install:[/dim]     " + product_command("plugins") + f" install {entry.name}")
    console.print()

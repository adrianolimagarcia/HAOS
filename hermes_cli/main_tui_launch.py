...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``haos update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``haos update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        ...
        "This usually means `" + product_command("update") + "` left tracked ui-tui files deleted.\n" +
        "Recovery:\n" +
...
def _pin_kanban_board_env() -> None:
    ...
    ``haos kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``haos kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``haos kanban boards switch`` from another session can flip the file mid-turn, so the same
...
def _resolve_use_tui(args) -> bool:
    ...
    ``haos chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and

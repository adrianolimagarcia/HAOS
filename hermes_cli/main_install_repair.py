...
def _stdout_to_stderr():
    ...
    a protocol stream (``haos acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``haos update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from hermes_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `" + product_command("update") + "` was interrupted mid-install — " +
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``haos update`` so the sync and its tail happen
...
def _quarantine_running_hermes_exe(
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            ...
            "gateway, or pause AV scanning, then re-run `" + product_command("update") + "`.")
...
def _repair_venv_via_import_probes(
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `" + product_command("update") + "`:")
...
def _verify_console_scripts_installed(
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `" + product_command("update") + " --force` after " +
...
def _verify_core_dependencies_installed(
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `" + product_command("update") + "` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `" + product_command("update") + " --force` after closing other hermes processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `" + product_command("update") + " --force` after closing other hermes processes.",

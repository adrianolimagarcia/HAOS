...
connectors: ``haos auth upgrade`` in a terminal and ``/login`` inside a chat.
...
UPGRADE_HINT = "Run `" + product_command("auth") + " upgrade` to sign in with a Nous account, or /login inside a chat."
FREE_TIER_NOT_SIGNED_IN = (
    "You're not signed in. Free inference and connectors are always on. "
    "Run `" + product_command("auth") + "` to sign in with a Nous account.")
...
_SIGNIN_TERMINAL = "To sign in: `" + product_command("auth") + " upgrade`."
_MODEL_HINT_CHAT = "Run /model and pick the Nous row again."
_MODEL_HINT_TERMINAL = "Run `" + product_command("model") + "` and pick the Nous row again."
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    map. The config write is the same one a sign-in completion uses, so ``haos model`` and the
...
# --- ``haos auth upgrade``: sign the guest into a real Nous account, keeping its connectors ---------
#
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Every sign-in completion (CLI ``haos auth upgrade``, the desktop poller) calls this once, after
    ...
    try:
        ...
        # same write; the runtime's silent default applies until the user picks one with `haos model`.
        _update_config_for_provider(

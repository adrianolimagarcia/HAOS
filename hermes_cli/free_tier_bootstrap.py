...
Every Hermes process that may need the free tier runs this once at boot (``haos serve`` on a
...
def start_background_bootstrap() -> threading.Thread:
    """``hermes/haos serve`` entry: run on a daemon thread so a slow portal never delays the socket."""

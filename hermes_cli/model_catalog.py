...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``haos update`` that file IS the newest catalog, so the picker stays current even when
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``haos model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime, _catalog_cache_source_path
    _catalog_cache = None

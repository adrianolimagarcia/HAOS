...
class SessionDB(
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"`" + product_command("sessions") + f" recover --source {qpath or self.db_path} --inspect-only`, or restore a "
            "snapshot with `/snapshot list` then `/snapshot restore <id>` (terminal `" + product_cli_name() + "` chat only)."
        )
        logger.error(msg)

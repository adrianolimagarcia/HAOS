...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # scoped per profile so `haos cron status` reflects liveness for every profile independently.
        if profile_homes:
        ...
        # Heartbeat before the first sleep so `haos cron status` sees a live ticker immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `haos cron status` (separate process) shows WHY.

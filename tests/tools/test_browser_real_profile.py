...
class TestReviewRound3:
    ...
    def test_relaunch_path_does_snapshot(self, tmp_path):
        ...
        def fake_popen(argv, **kw):
            # The relaunch path really starts the browser binary. Without this fake it execs
            # the patched path, which fails on a host with no Chrome installed — making the
            # test depend on the host's browser instead of on the snapshot decision it pins.

...
def test_slot_runtime_cache_expires_after_ttl(monkeypatch):
    ...
    # Age the entry past the TTL and confirm re-resolution. The cache key is
    # profile-scoped — (hermes_home_key(), provider, model) — because the resolved
    # api_key/base_url are per-profile and two profiles can share (provider, model).
    from hermes_constants import hermes_home_key
    key = (hermes_home_key(), "openai", "gpt-5")
    assert key in moa._runtime_cache, sorted(moa._runtime_cache)

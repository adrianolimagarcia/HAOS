    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if cause == "corrupt":
            ...
            message = (
                ...
                f"recovery tools or restore a backup unless `{cli}{profile_arg}doctor` confirms damage."
            )
        else:
            ...
            message = (
                ...
                f"empty. Cause: {failure.gloss}. Run `{cli}{profile_arg}doctor --fix` on the "
                f"gateway machine, then `{cli}{profile_arg}gateway restart`."
            )

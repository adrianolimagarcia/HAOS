...
class SessionSchemaMixin:
    ...
    def _defer_stale_fts_for_holders(self, cursor: sqlite3.Cursor, foreign_holders) -> bool:
        ...
        if futile:
            logger.error(
                ...
                "own retry admits the rebuild within %.0fs of the holder leaving. `" + product_command("doctor") + "` shows this.",
            ...
            logger.error(
                ...
                "processes (this process's own retry then rebuilds), or run `" + product_command("sessions") + " optimize-storage` "
                "with every holder stopped. `" + product_command("doctor") + "` reports this degraded state.", attempts, foreign_holders,
            )
        logger.warning(
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        # install only gets a flag; `haos sessions optimize-storage` performs it. The FTS

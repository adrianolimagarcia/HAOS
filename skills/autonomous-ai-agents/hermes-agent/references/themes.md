...
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `haos config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   haos config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `haos config set display.skin default` (or they can `/skin default`).
...
```
haos skin set <key> <hex>      # e.g. haos skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `haos skin set background "#08201f"` changes
only the background; `haos skin use <name>` / `haos skin list` switch and
enumerate.
...
- **Never hand-edit `config.yaml` to activate.** Use `haos config set
...
- **You apply it, not the user.** `haos config set display.skin <name>` is
...
- Run `haos config get display.skin` and confirm it reports `<name>`.

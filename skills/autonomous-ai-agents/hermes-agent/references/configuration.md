...
Edit with `haos config edit` or `haos config set section.key value`.
...
`haos config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `haos tools` (interactive) or `haos tools enable/disable NAME`.

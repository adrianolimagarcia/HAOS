...
the `cronjob` tool, the `haos cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `haos curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `haos curator run --consolidate`. Routine background curation costs
  zero tokens.
...
Users drive it via `haos kanban <verb>`; dispatcher-spawned workers

...
the Hermes CLI, TUI, and desktop app. This skill drives the `haos pets` CLI
...
Use the `terminal` tool to run `haos pets <subcommand>`.

## Quick Reference

| Goal | Command |
| --- | --- |
| Browse the gallery | `haos pets list` (add a substring to filter: `haos pets list cat`) |
| List installed pets | `haos pets list --installed` |
| Install a pet | `haos pets install <slug>` (add `--select` to make it active) |
| Set the active pet | `haos pets select <slug>` (omit slug for a picker) |
| Resize the pet everywhere | `haos pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate in terminal | `haos pets show [slug] [--cycle] [--state run]` |
| Disable the pet | `haos pets off` |
| Remove a pet | `haos pets remove <slug>` |
| Diagnose setup | `haos pets doctor` |

## Procedure

1. Find a pet: `haos pets list <query>` and note its `slug`.
2. Install + activate: `haos pets install <slug> --select`.
3. Preview it: `haos pets show` (Ctrl+C to stop).
4. Confirm setup: `haos pets doctor` — shows the resolved pet, configured
...
- `scale` (float) — on-screen size of the native 192×208 frames (default 0.33,
  ...
  `haos pets scale <factor>`, the `/pet scale` slash command, or the desktop
  Appearance slider.
...
- The petdex npm CLI installs to `~/.codex/pets`; Hermes uses its own
  profile-scoped `<HERMES_HOME>/pets/` instead — install through `haos pets`.

## Verification

- `haos pets doctor` reports `✓ ready` when a pet is installed, selected,

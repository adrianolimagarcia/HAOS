...
Set via `haos model` (picker) or `haos setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `haos auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `haos auth add openai-codex` |
| qwen-oauth | OAuth | `haos auth add qwen-oauth` |
| minimax-oauth | OAuth | `haos auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`haos auth`).
Fallback chain when the primary fails: `haos fallback add|remove|list`.
...
#   haos config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:

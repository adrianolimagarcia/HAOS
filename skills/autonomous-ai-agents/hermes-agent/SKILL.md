...
| MCP servers (add, catalog, `haos mcp`) | `references/native-mcp.md` |
...
Two theming rules that hold even without loading the reference: **you apply skins yourself** (`haos config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`haos skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.
...
```
terminal(command="haos chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="haos chat -q 'Set up CI/CD for ~/myapp'", background=true)
```
...
- **Use `haos chat -q` for fire-and-forget** — no PTY needed
...
- **Never hand-edit `config.yaml` for the user** — use `haos config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.

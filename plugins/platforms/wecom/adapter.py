...
_ACCESS_CHOICES = (
    ...
    ("Use DM pairing (unknown users request access, you approve with '" + product_command("pairing") + " approve')", (("WECOM_DM_POLICY", "pairing"),),
     (("success", "DM pairing mode — users will receive a code to request access."), ("info", "Approve with: " + product_command("pairing") + " approve <platform> <code>"))),
    ("Disable direct messages", (("WECOM_DM_POLICY", "disabled"),), (("warning", "Direct messages disabled."),)),
    ("Skip for now (bot will deny all users until configured)", (), (("info", "Skipped — configure later with '" + product_command("gateway") + " setup'"),)),
)
...
def register(ctx) -> None:
    common = dict(install_hint="Run `" + product_command("setup") + "` to install WeCom support.", emoji="💼", allow_update_command=True)

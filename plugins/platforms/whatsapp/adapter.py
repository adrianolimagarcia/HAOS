...
from hermes_constants import (find_node_executable, get_hermes_dir, with_hermes_node_path, product_command)
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _ensure_bridge_deps(self, bridge_dir: Path) -> bool:
        ...
        self._set_fatal_error("whatsapp_npm_install_failed", f"WhatsApp bridge npm install failed{detail}. Run `cd {bridge_dir} && {_npm_bin} install` "
                              "manually, then restart `" + product_command("gateway") + "`.", retryable=False)
        return False
    ...
    async def _wait_for_bridge(self) -> bool:
        ...
        if data.get("status") != "connected":
            ...
            if connected is None:
                ...
                print(f"[{self.name}]   If session expired, re-pair: {product_command('whatsapp')}")
        return True
    ...
    def _preflight(self) -> bool:
        ...
        checks = (
            (check_whatsapp_requirements, ("[%s] Node.js not found. WhatsApp requires Node.js.", self.name),
             "whatsapp_node_missing", "Node.js is not installed — install Node.js and re-run `" + product_command("gateway") + "`."),
            ...
            (creds_path.exists, ("[%s] WhatsApp is enabled but not paired (no creds.json at %s). Pair from the dashboard or run " +
                                 "`" + product_command("whatsapp") + "`; remove WHATSAPP_ENABLED from your .env to disable.", self.name, creds_path),
             "whatsapp_not_paired", "WhatsApp enabled but not paired — pair from the dashboard or run `" + product_command("whatsapp") + "`."),
        )

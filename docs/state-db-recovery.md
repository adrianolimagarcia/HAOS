...
`haos doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `haos doctor --fix` while the gateway
is still up. Next steps:

```bash
haos gateway stop
HERMES_HOME="$HOME/.hermes" haos sessions recover --source "$HOME/.hermes/state.db" --inspect-only
# if recoverable:
HERMES_HOME="$HOME/.hermes" haos sessions recover --source "$HOME/.hermes/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
haos gateway stop
HERMES_HOME="$HOME/.hermes" haos sessions repair --check-only
HERMES_HOME="$HOME/.hermes" haos sessions repair
```

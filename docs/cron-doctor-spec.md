...
a provider run fails, or delivery starts failing. `haos cron list` shows some of
...
Add `haos cron doctor` as a read-only diagnostic command that summarizes cron
...
- `haos cron doctor` returns `0` and prints a healthy message when active jobs

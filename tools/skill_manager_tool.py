...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if unsafe := _validate_delete_target(skill_dir):  # defense-in-depth before rmtree
        return _err(unsafe)
    # Curator consolidations must be RECOVERABLE (`haos curator restore`): archive instead
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
    ...
    # record as STATE_ARCHIVED (`haos curator status`/`restore`); only a hard delete forgets.
    with suppress(Exception):
        ...
        # and `haos curator restore` promises the skill can be brought back. Route through the recoverable

...
from hermes_constants import get_bundled_skills_dir, get_hermes_home, get_optional_skills_dir, product_command
...
# Written by `haos profile create --no-skills` / installer `--no-skills`: sync seeds only
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `{product_command('skills')} reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `{product_command('skills')} reset {skill_name}` to "

...
def unavailable_hint(name: str) -> str:
    """Branded install/login hint for a target with no approval surface in this session.

    Lazy on purpose: the product CLI name comes from the runtime environment
    (``is_haos_environment()``), so a module-level constant would freeze the brand at import."""
    cmd = product_command("mcp")
    return f"{cmd} install {name} / {cmd} login {name}"
...
def _unavailable(operation: ConnectionOperation) -> str:
    for target in operation.targets:
        ...
        target.extra = {"hint": unavailable_hint(target.name)}

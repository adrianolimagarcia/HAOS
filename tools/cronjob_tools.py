...
def _reap_stale_executions(job_name: str) -> None:
    ...
    try:
        ...
        # process -- e.g. a PRIOR one-shot `haos cron run` invocation whose dispatched runner died with
...
def _try_dispatch_background_run(
    ...
    # completion, so don't claim-and-dispatch (direct callers like `haos cron run` exit right after).
...
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `haos cron

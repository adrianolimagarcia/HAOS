...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        descendant (e.g. a daemon from ``haos update``) holds the pipe open, poll()
        ...
        process (e.g. a daemon spawned by `haos update` restarting the gateway) is still holding the

...
subagent_auto_approve, inherit_mcp_toolsets, max_iterations, require_verification_before_spawn`
(the last one, HAOS P9, refuses new spawns while the parent has unverified turn edits — see
`tools/delegate_verification_gate.py`). **Child processes:** a child's background

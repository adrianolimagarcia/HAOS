...
def browser_vault_save_login(label: str = "", task_id: Optional[str] = None) -> str:
    ...
    if prompt is None or not can_prompt_here():
        return json.dumps({"success": False, "error_type": "prompt_unavailable",
                           "error": (f"This session cannot ask the user for a login (headless/cron/API). Tell them to run `" + product_command("vault") +
                                     f" add` or use Desktop → Settings → Passwords & Logins for {origin}.")})
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    "To save a credential: run `" + product_command("vault") + " add` in a terminal, or "
...
BROWSER_VAULT_SAVE_LOGIN_SCHEMA = {
    ...
    "description": (
        ...
        "Logins / `" + product_command("vault") + " add`."
    ),
    "parameters": {
        "type": "object",

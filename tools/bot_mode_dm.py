...
local → ``haos -p <name> chat --in ~ -c "Bot Chat" --create-if-missing -Q
--query-file <tmp>``; peer → ``haos peer dm <peer>[/<name>] < <tmp>``; both via
...
def _run_delivery(argv: list[str], dm_file: str, *, stdin_file: bool,
    ...
    ``author`` rides to the child as HERMES_TURN_AUTHOR; ``haos peer dm`` forwards it in the request body.

...
def _ensure_docker_available() -> None:
    ...
    if not docker_exe:
        raise _docker_unavailable(
            ...
            hint="Install Docker (or fix PATH) and retry, or run `" + product_command("setup") + " terminal` to switch to Local.")
    try:
        ...
        raise _docker_unavailable(
            ...
            hint="Start Docker (e.g. `systemctl start docker` or launch Docker Desktop), then retry — "
                 "or run `" + product_command("setup") + " terminal` to switch to Local.")
    except Exception:

...
(`haos-gateway-<name>.service` / `hermes-gateway-<name>.service`), a systemd **system** service when installed with
`sudo hermes/haos gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `hermes/haos serve` backend. This guide adds the patterns for managing them
collectively.

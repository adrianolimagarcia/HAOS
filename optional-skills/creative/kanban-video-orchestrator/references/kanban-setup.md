...
2. **Create profiles** — `haos profile create <name> --clone`
...
6. **Fire the initial kanban task** — `haos kanban create` assigned to the director
...
```bash
haos profile create director --clone 2>/dev/null || true
```
...
```bash
haos kanban create "Direct production of <video title>" \
    --assignee director \

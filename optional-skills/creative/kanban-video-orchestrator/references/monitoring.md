...
haos kanban watch --tenant <project-slug>

# Snapshot of the board
haos kanban list --tenant <project-slug>
haos kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
haos kanban stats --tenant <project-slug>

# Visual dashboard (browser)
haos dashboard

# Inspect a specific task (includes comments + events)
haos kanban show <task-id>

# Follow a single task's event stream
haos kanban tail <task-id>
```

Verify available subcommands with `haos kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `haos kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `haos kanban block <id>` then `haos kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `haos kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `haos kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `haos profile list` (profile exists?), `haos status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(haos kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
haos kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
haos kanban block <task-id>
haos kanban archive <task-id>

# Diagnose what happened
haos kanban show <task-id>      # task body, comments, recent events
haos kanban tail <task-id>      # follow the live event stream
haos kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `haos kanban create` for the director
...
while true; do
    clear
    haos kanban list --tenant <slug>
    echo "---"
    haos kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `haos kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `haos profile list` and start

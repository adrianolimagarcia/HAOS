...
3. Creates each Hermes profile via `haos profile create --clone`
...
7. Fires the initial `haos kanban create` task assigned to the director
...
```bash
haos kanban watch --tenant <project-tenant>     # live events
haos kanban list  --tenant <project-tenant>     # board snapshot
haos dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `haos kanban create` call to a specialist profile. The kanban

...
- An image-generation backend configured for the built-in `image_generate` tool; check with `haos tools` and enable one there if the tool reports no backend.
...
8. Determine the available image-generation path before promising output. In Hermes this is the `image_generate` tool; if it reports no configured backend, ask the user to enable one (`haos tools`) instead of fabricating results.

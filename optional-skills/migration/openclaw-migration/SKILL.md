...
```bash
haos claw migrate              # Full interactive migration
haos claw migrate --dry-run    # Preview what would be migrated
haos claw migrate --preset user-data   # Migrate without secrets
haos claw migrate --overwrite  # Overwrite existing conflicts
haos claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `haos setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
